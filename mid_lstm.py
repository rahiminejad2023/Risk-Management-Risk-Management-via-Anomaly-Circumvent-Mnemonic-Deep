...
filename=np.load('Data\\filename_delete_sort.npy')
result_Mid_LSTM_df= pd.DataFrame()
#result_Mid_LSTM_df.columns = ['index', 'stock', 'x_linear_test', 'x_linear_train', 'y_linear_test', 'y_linear_train']


n=len(filename)
# n=10
if os.path.isfile('Data/Mid_LSTM_451.npy'):

  result_Mid_LSTM_df1=np.load('Data/Mid_LSTM_451.npy', allow_pickle=True)
 # result_Mid_LSTM_df = pd.DataFrame(columns=('index', 'stock', 'x_train', 'y_train', 'x_test', 'y_test'))

  result_Mid_LSTM_df=pd.DataFrame(result_Mid_LSTM_df1)
...
for i in range(0,n):
  index=i
  stock=filename[i]
  data2=len(result_Mid_LSTM_df.loc[result_Mid_LSTM_df['stock'] == filename[i]])
  #data2=data1.['stock'].isin(filename[i]))
  if data2==0:
      print("start stock:"+filename[i]+str(i))
      result=getdata_loop("Data\\"+filename[i])
      x_train=result[0]
      y_train=result[1]
      x_test=result[2]
      y_test=result[3]
      result_Mid_LSTM_df=result_Mid_LSTM_df.append(pd.DataFrame({'index':[index],
                                                         ...
                                                         'x_linear_test':[x_test],
                                                         'y_linear_test':[y_test]}),ignore_index=True)
      print(i)
      np.save('Data/Mid_LSTM_451.npy', result_Mid_LSTM_df)

data=np.load('Data/Mid_LSTM_451.npy', allow_pickle=True)
data=pd.DataFrame(data)
data.columns = ['index','stock','x_linear_train','y_linear_train','x_linear_test','y_linear_test']
...
#train product
    ...
    df1['hidden_train']=data['x_linear_test'][i]['hidden_test']
    df1['train_corr']=data['x_linear_test'][i]['test_corr']
    df1['train_stock_pre']=data['x_linear_test'][i]['test_stock_pre']
    ...
    np.save('Data\\Mid_LSTM_new.npy',result_Mid_LSTM_df)

#Reshape data

temp_df=np.load('Data\\Mid_LSTM_new.npy',allow_pickle=True)
temp_df=pd.DataFrame(temp_df)
# temp_df.columns=['MSE','TRUE','accuracy','index','predict','stock']
temp_df.columns=['index','stock','MSE','accuracy','predict','TRUE']
...
result_df=pd.DataFrame(columns=('index','stock','MSE','accuracy','TRUE','predict'))
...
for i in range(0,n):
  ...
  #predict
  t=[]
  help=temp_df['predict'][i]
  for j in range(0,len_pre):
    # print(str(i) + ":" + str(j))
    try:
      t.append(temp_df['predict'][i][j])
    except:
      t.append(temp_df['predict'][i])
  ...
  result_df=result_df.append(pd.DataFrame({'index':index,'stock':[stock],
                             ...
                             ignore_index=True)
  print(i)
np.save('Data\\Mid_LSTM_r.npy',result_df)

##Mean MPA of all stocks
temp_r=np.load('Data\\Mid_LSTM_r.npy',allow_pickle=True)
temp_r=pd.DataFrame(temp_r)

temp_r.columns=['index','stock','MSE','accuracy','TRUE','predict']
n=451
avg_accuracy1=[]
...
filename=np.load('Data\\filename_delete_sort.npy')#Stock names
sort=np.load('Data\\new_sortname.npy',allow_pickle=True)#Stock names ranked according to correlation coefficient from high to low

LH3pro_r=np.load('Data\\Mid_LSTM_r.npy',allow_pickle=True)
LH3pro_r=pd.DataFrame(LH3pro_r)
# LH3pro_r.columns=['MSE','TRUE','accuracy','index','predict','stock']
LH3pro_r.columns=['index','stock','MSE','accuracy','TRUE','predict']
...
for i in range(0,n):
  for j in range(0,top):
    if LH3pro_r_ori['stock'][i]==sort[j]:
        help2=LH3pro_r_ori.iloc[i]
        LH3pro_r.append(help2)
LH3pro_r=LH3pro_r_ori
...
np.save('Data\\LH3pronew_a_50.npy',avg_accuracy10)
