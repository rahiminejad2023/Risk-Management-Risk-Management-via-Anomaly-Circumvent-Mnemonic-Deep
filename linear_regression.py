...
#   MSE=stock_loop(filename_list[i])
  ...
  true=result[2]
  predict=result[3]
  result_linear_df=result_linear_df.append(pd.DataFrame({'index':[index],  'stock':[stock],'MSE':[MSE],'accuracy':[accuracy],'true':[true], 'predict':[predict]}),ignore_index=True)
  print(i)
  np.save('Data\\linear_d0.npy',result_linear_df)
  result_linear_df.to_csv('Data\\linear_d0.csv')
'''
#Reshape datar
L4_all=np.load('linear_d0.npy',allow_pickle=True)
L4_all=pd.DataFrame(L4_all)
# L4_all.columns=['MSE','accuracy','index','predict','stock','TRUE']
L4_all.columns=['index','stock','MSE','accuracy','predict','TRUE']
...
for i in range(0,n):
  ...
  t=[]
  for j in range(0,len_pre):
    t.append(L4_all['TRUE'][i][j])
  TRUE=t
  #predict
  t=[]
  help=L4_all['predict'][i]
  for j in range(0,len_pre):
    print(str(i)+":"+str(j))
    try:
      t.append(L4_all['predict'][i][j])
    except:
      t.append(L4_all['predict'][i])
  ...
  result_df=result_df.append(pd.DataFrame({'index':index,'stock':[stock],
                             ...
                             ignore_index=True)
  print(i)
np.save('Data\\linear_r.npy',result_df)
##Mean MPA of all stocks
linear_r=np.load('Data\\linear_r.npy',allow_pickle=True)
linear_r=pd.DataFrame(linear_r)
#linear_r.columns=['MSE','TRUE','accuracy','index','predict','stock']
linear_r.columns=['index','stock','TRUE','predict','accuracy','MSE']
