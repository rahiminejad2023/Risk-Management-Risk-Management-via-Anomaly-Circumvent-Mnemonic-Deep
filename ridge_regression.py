...
for i in range(0,n):
  index=i
  stock=filename[i]
  result=stock_ridge_loop("Data\\"+filename[i])
  MSE=result[0]
  accuracy=result[1]
  true=result[2]
  predict=result[3]
  ...
  print(i)
  np.save('Data\\ridge_d0.npy',result_ridge_df)
  result_ridge_df.to_csv('Data\\ridge_d0.csv')
'''
#Reshape data
L4_all=np.load('Data\\ridge_d0.npy',allow_pickle=True)
...
for i in range(0,n):
  ...
  result_df=result_df.append(pd.DataFrame({'index':index,'stock':[stock],
                             ...
                             ignore_index=True)
  print(i)
np.save('Data\\ridge_r_new.npy',result_df)


##Mean MPA of all stocks
ridge_r=np.load('Data\\ridge_r_new.npy',allow_pickle=True)
ridge_r=pd.DataFrame(ridge_r)
#ridge_r.columns=['MSE','TRUE','accuracy','index','predict','stock']
ridge_r.columns=['index','stock','TRUE','predict','accuracy','MSE']
